...
max_h(INT_MIN), min_h(INT_MAX) {
    int defauld_value = 0;
    this->data = init(MAX, defauld_value);
    this->cumsum = init(MAX, defauld_value);
}
...
Histogram Histogram::clone() const {
    ...
    _copy.data = copy(data, 256);
    _copy.cumsum = copy(cumsum, 256);

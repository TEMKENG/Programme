...
%	\textcolor{red}{
%		\begin{itemize}
%			\item Faltungsneuronale Netze sind in der Computervision allgegenwärtig geworden, seit AlexNet [19] durch den Gewinn der ImageNet Challenge: ILSVRC 2012 [24] tiefe Faltungsneuronale Netze populär gemacht hat. Der allgemeine Trend geht dahin, tiefere und kompliziertere Netze zu erstellen, um eine höhere Genauigkeit zu erreichen [27, 31, 29, 8]. Diese Fortschritte zur Verbesserung der Genauigkeit machen die Netze jedoch nicht unbedingt effizienter in Bezug auf Größe und Geschwindigkeit. 
%			\item In den letzten Jahren haben tiefe neuronale Netze bei verschiedenen anspruchsvollen Aufgaben in den Bereichen Computervision, Spracherkennung und Verarbeitung natürlicher Sprache den neuesten Stand der Technik erreicht. Angetrieben durch steigende Datenmengen und Rechenleistung sind tiefe Lernmodelle größer und tiefer geworden, um besser aus Daten zu lernen. für die On-Device-Inferenz stellt einige zentrale Herausforderungen dar. Erstens haben moderne Modelle für tiefes Lernen routinemäßig Millionen von Parametern, die MBs Speicherplatz benötigen, während der Speicher auf dem Gerät begrenzt ist. Darüber hinaus ist es nicht ungewöhnlich, dass selbst eine einzige Modellschlussfolgerung Milliarden von Speicherzugriffen und Rechenoperationen hervorruft, die alle Strom verbrauchen und Wärme abgeben, die die begrenzte Batteriekapazität erschöpfen und/oder die thermischen Grenzen des Geräts testen können.Angesichts dieser Herausforderungen ist eine wachsende Zahl von Arbeiten entstanden, die darauf abzielen, Methoden zur Komprimierung von Modellen neuronaler Netze zu finden und gleichzeitig den möglichen Verlust an Modellqualität zu begrenzen. Latenzempfindliche Arbeitslasten, die sich auf Inferenz auf Geräten mit niedrigem Stromverbrauch stützen, hängen oft mit der Speicherbandbreite zusammen, und die Modellkomprimierung bietet den doppelten Vorteil, dass die Gesamtzahl der leistungsintensiven Speicherzugriffe reduziert und die Inferenzzeit durch eine effektiv höhere Speicherbandbreite zum Abrufen komprimierter Modellparameter verbessert wird. 
%			\item In der jüngeren Literatur ist das Interesse am Aufbau kleiner und effizienter neuronaler Netze gestiegen. MobileNets konzentrieren sich in erster Linie auf die Optimierung der Latenz, aber auch auf kleine Netzwerke.
%		\end{itemize}
%	}
%	
\end{itemize}
...
\begin{enumerate}
	...
	\begin{enumerate}
		\item \textbf{ Food-101-original (Food-101-O)} ist ein Datensatz von $ 101 $ Lebensmittelkategorien mit $ 101.000 $ Bilder. Für jede Klasse werden 250 manuell überprüfte Testbilder sowie 750 Trainingsbilder bereitgestellt. Die Trainingsbilder wurden bewusst nicht gereinigt und enthalten daher noch etwas Rauschen. Dies geschieht meist in Form von intensiven Farben und manchmal falschen Etiketten. Alle Bilder wurden so skaliert, dass sie eine maximale Seitenlänge von 512 Pixel aufweisen.
	  	\item\textbf{ Food-101 von Tensorflow: (Food-101-T)} Dieser Datensatz enthält die gleiche Anzahl von Bildern wie der soeben beschrieben, Aber der Datensatz wird nicht aufgeteilt und nicht gereinigt und enthalten daher noch etwas Rauschen. Dies geschieht meist in Form von intensiven Farben und manchmal falschen Etiketten.Für unsere Arbeit wurde jede Kategorie in drei(Trainings-, Auswertungs- und Testdatensatz) aufgeteilt ist, die $ 800, 100 $ bzw. $ 100 $ Bilder umfasst.
	\end{enumerate}
...
\subsubsection{Anzahl der Feature-Maps pro Schicht:}
...
\textcolor{red}{
	\begin{itemize}
		\item Als grobe Faustregel gilt, dass Ihr Modell an mindestens einer Größenordnung mehr Beispiele als trainierbare Parameter trainieren sollte.
	\end{itemize}
}
...
\subsection{Verringerung  des Speicherbedarfs }
...
\subsubsection{Extreme Version von \textit{TemkiNet.}}
...
\end{table}
 Die erste Änderung reduziert die Anzahl der Parameter und beeinträchtigt ein Bisschen die Netzwerkleistung, denn eine Faltungsschicht mit einer Schrittweite von 2 ermöglicht nicht die Änderung der räumlichen Dimension, sondern auch eine Extraktion von Merkmalen. 
 
 Die zweite Änderung stellt sicher, dass  die Größe des Modells unabhängig von der Größe des Netzwerkeingangs gleich bleibt. Ein großer Vorteil ergibt sich aus diesem Ansatz bei der Verwendung großer Bilder(siehe Tabelle \ref{tab:Bildqualitaetf}).
 
 Was die letzte Änderung betrifft, so erlaubt sie eine drastische Reduzierung der Größe des Klassifikationsblocks von $ 935.936 $ auf etwa $ 126.553 $ Parameter.
\begin{table}[h!]
	\centering
	\begin{tabular}{*{3}{|c}|}
		\hline
...
\begin{table}[h!]
	...
	\label{tab:pruning}
\end{table}
Es ist wichtig zu beachten, dass ein spärliches Modell nicht schneller als das ursprüngliche Modell ausgeführt werden können wird. Allerdings die Bibliotheken wie \textit{TensorFlow Lite oder Sparse} nutzen diese Sparsamkeit aus, um die Berechnungen zu beschleunigen. Hinzu kommt, dass der Verlust an Genauigkeit umso größer ist, je spärlicher das CNN ist. Dieser Verlust kann jedoch durch die einfache Verlängerung der Trainingszeit des CNN wiederhergestellt werden.
...
Zusammenfassend lässt sich hier sagen, dass die Quantifizierung Vorteile wie eine deutliche Reduzierung des Speicherbedarfs und eine mögliche Beschleunigung der Inferenz für bestimmte Hardware aufweist und angesichts seiner Bedeutung wäre es vorteilhaft, wenn nicht nur einige Systeme, sondern alle Systeme davon profitieren würden.

\subsection{Transfer-Lernen (\textit{Transfer Learning})}
Der größte Nachteil von \textit{TemkiNet} ist seine lange Trainingszeit und das kann in der Abbildungen [\ref{fig:DataAugmentation}, \ref{fig:units}] gesehen werden. Um diese Trainingszeit zu reduzieren, greifen wir zur Technik des Transfer-Lernens.

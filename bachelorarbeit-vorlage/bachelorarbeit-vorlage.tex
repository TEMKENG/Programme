...
In einem \ac{ConvL} wird die sogenannte Faltungsoperation(\emph{convolution operation}) durchgeführt,dabei wird das komponentenweises Produkt(\textit{Hadamard-Produkt}) zwischen einem kleinen Bereich der Eingabedaten und einem Kernel durchgeführt und dann die ganze aufsummiert. Eine Illustration der Faltungsoperation ist in der Abbildung \ref{Faltungsoperation2} zu sehen. Die Resultante der Faltungsoperation wird die Aktivierung des Neurons genannt.Sollte diese Aktivierung null sein, dann sagt man, dass das Neuron "\textit{nicht aktiv}"{} ist und sonst ist das 
...
\begin{itemize}
	...
	\item \textbf{\textit{Padding}}\\
		Mit einem großen Filter lernt man in der Regel "mehr"{} als mit einem kleinen.Aber wie in Tabelle \ref{tab:Stride and filter size} zu sehen, habe man eine Reduktion der Dimension, wenn ein großes Filter angewendet wird und dieser Verlust der Dimension ist nicht immer erwünscht, denn das verhindert, dass tiefere \acsp{CNN} entworfen werden und um die Dimension zu behalten, wird vor der Anwendung der Filter auf das Bild eine Füllung(\textit{padding}) an den Rändern des Bilds gemacht. Standard wird das Bild mit Nullen gefüllt und kann jedoch auch mit anderen Werten gefüllt werden.

\end{itemize}

Bei der Faltungsoperation wird 


Sehr oft sagt man, dass ein Neuron zwei Zustände "{}aktiv"{} und "nicht aktiv"{} hat\\
\textbf{\emph{Check liste}}
\begin{enumerate}
	\item Filter ok
	\item stride ok
	\item padding ok
	\item receptive field ok
	\item low and higher level features
	\item activate? yes or no in Filter

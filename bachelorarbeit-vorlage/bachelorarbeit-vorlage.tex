...
Es wird von Quantisierung gesprochen, wenn es versucht wird, sich den Werten einer großen Menge mit denen einer kleineren Menge zu nähern
\begin{center}
	\textbf{Methoden}
	\begin{itemize}
...
\begin{equation}\label{eq:SVD}
	...
	{||A||_F} = \sqrt{\sum_{i = 1}^{m} \sum_{j =1}^{n} A_{i,j}^2} := \text{Frobenius Norm}
	\end{array}	
\end{equation}
...
\end{figure}
Wie man in der Abbildung \ref{fig:matrix_fatorization} sieht, reichen  relativ wenige Singulärwerte (ca. $ 5 $) schon aus, um die groben Konturen zu erkennen. Details erkennt man bei ca. $ 10 $ Singulärwerten schon. Generell wird das Bild schärfer bei mehr Singulärwerten.
